...
//我只注释常用的一些函数  剩下的如果有兴趣请自行研究
//此程序将每一行设置为24个像素 共10行

void LCD_Init(void);                                                    //LCD初始化函数
void LCD_SetTextColor(vu16 Color);                                      //设置前景色(文字颜色)
void LCD_SetBackColor(vu16 Color);                                      //设置背景色
void LCD_ClearLine(u8 Line);                                            //清空指定行
void LCD_Clear(u16 Color);                                              //清空整个屏幕并用指定颜色填充
void LCD_SetCursor(u8 Xpos, u16 Ypos);
void LCD_DrawChar(u8 Xpos, u16 Ypos, uc16 *c);                          //在指定坐标显示一个字符(如果要自行改造函数的话 可能会用到这个)
void LCD_DisplayChar(u8 Line, u16 Column, u8 Ascii);                    //在指定行的指定坐标处显示一个字符
